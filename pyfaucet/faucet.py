...
def index():
    ...
    tx = subprocess.run(["palomad", "--fees", "200grain", "--node", "tcp://testnet.palomaswap.com:26657", "tx", "bank", "send", "-y", BANK, address, "100000grain"], capture_output=True)
    if tx.stderr:
        print(tx.stderr)
        return "failed"
